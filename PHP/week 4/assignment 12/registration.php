...
<body class="registration-page">
        ...
        if(empty($_POST['username']) || empty($_POST['first_name']) || empty($_POST['last_name']) || empty($_POST['password']) || empty($_POST['confirm_password']) || empty($_POST['email']) || !isset($_POST['accept_terms'])) {
            ...
            if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                ...
                if ($password !== $confirm_password) {
                    ...
                    // Sanitize inputs
                    $username = mysqli_real_escape_string($database_connect, strip_tags($_POST['username']));
                    $first_name = mysqli_real_escape_string($database_connect, strip_tags($_POST['first_name']));
                    $last_name = mysqli_real_escape_string($database_connect, strip_tags($_POST['last_name']));
                    $password = mysqli_real_escape_string($database_connect, strip_tags($_POST['password']));
                    $email = mysqli_real_escape_string($database_connect, strip_tags($_POST['email']));
                    $province = mysqli_real_escape_string($database_connect, strip_tags($_POST['province']));
    ...
    <div id="form-div">
        <form class="form" method="post">
            ...
            <label for="email">Email:</label> <input type='text' id='email' name='email' placeholder='Enter your Email...' value='<?php echo isset($_POST["email"]) ? htmlspecialchars($_POST["email"]) : ""; ?>'><br>
